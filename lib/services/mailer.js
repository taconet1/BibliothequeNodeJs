...
module.exports = class MailerService extends Service {
    ...
    async sendNotificationModifyMovie(email) {
        const transporter = await this.getTransporter();
        const info = await transporter.sendMail({
            from: '"Fred Foo 👻" <foo@example.com>', // sender address
            bcc: email, // list of receivers
            subject: 'Modify Movie ✔', // Subject line
            text: 'We have modified the movie!', // plain text body
            html: '<b>We have modified the movie!</b>' // html body
        });
